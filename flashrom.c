	 ...
	 int write_it, int erase_it, int verify_it)
{
	...
	int ret = 0;
	unsigned long size = flash->chip->total_size * 1024;
//> sheldon : don't read all, just read the area specified in layout file.
//-	int read_all_first = 1; /* FIXME: Make this configurable. */
  int read_all_first = 0;
//<
...
//>sheldon: to fix unable to flash BIOS when ME is locked --> use build_new_image2 instead.
//-	if (build_new_image(flash, read_all_first, oldcontents, newcontents)) {
	if (build_new_image2(flash, read_all_first, oldcontents, newcontents)) {
//<
...
//> sheldon: we don't need to verify whole size, just need to verify ranges specified by user.
//-   ret = verify_range(flash, newcontents, 0, size);
//+
      {
        unsigned int start = 0;
        unsigned int included_start = 0, included_end = 0;
        while (start < size) {
          ret = get_next_included_range(flash, start, &included_start, &included_end, NULL);
          if (ret) {
            ret = 0;
            break;
          }
          // only compare the included ranges
          ret = verify_range(flash, newcontents + included_start, included_start, included_end - included_start + 1);
          if (ret) break;
          start = included_end + 1;
        }
      }
//+
//<
