...
//> sheldon : this is a new implemnt for copy_old_content !!
//+
static int copy_old_content2(struct flashctx *flash, int oldcontents_valid, uint8_t *oldcontents, uint8_t *newcontents, unsigned int start, unsigned int size)
{
  /* oldcontents is a zero-filled buffer. By reading the new data from file into oldcontents here, we
		 * avoid a rewrite of identical regions even if an initial full chip read didn't happen. */
	if (!oldcontents_valid) {
    memcpy(oldcontents + start, newcontents + start, size);
    return 0;
	}
	else {
    return copy_old_content(flash, oldcontents_valid, oldcontents, newcontents, start, size);
  }
}
//+
//<
